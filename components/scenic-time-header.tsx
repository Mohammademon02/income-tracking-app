...
export function ScenicTimeHeader({ className = "" }: ScenicTimeHeaderProps) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl shadow-2xl ${className}`}>
      ...
      <div className={`absolute top-4 sm:top-6 right-6 sm:right-12 w-8 h-8 sm:w-12 sm:h-12 ${scene.sunMoonColor} rounded-full shadow-lg`}>
      ...
      {/* Content Overlay */}
      <div className="relative z-10 bg-black/25 backdrop-blur-sm">
      ...
      {/* Time Period Indicator Tabs */}
      <div className="absolute bottom-2 sm:bottom-4 left-1/2 transform -translate-x-1/2">
        <div className="flex bg-black/20 backdrop-blur-sm rounded-lg p-1 border border-white/20">
          {["morning", "day", "sunset", "night"].map((period) => (
            <div
              key={period}
            ...
            >
              {period === "morning" ? "Morning" : period === "day" ? "Day" : period === "sunset" ? "Sunset" : "Night"}
            </div>
          ))}
        </div>
      </div>
